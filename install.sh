...
cd $HOME
sudo rm Nessus-* &>/dev/null </dev/null
sudo rm all-2.0* &>/dev/null </dev/null
echo //========================================================================
...
if [ "$installedPlugins" = "$vernum" ]; then
   ...
   sudo echo -e "PLUGIN_SET = \"${vernum}\";\nPLUGIN_FEED = \"ProfessionalFeed (Direct)\";\nPLUGIN_FEED_TRANSPORT = \"Tenable Network Security Lightning\";" | sudo tee /opt/nessus/var/nessus/plugin_feed_info.inc \&\> /dev/null
